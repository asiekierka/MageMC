...
public class MageCustomLightmap implements IMagePlugin {
	...
	private void applyCustomLightmap(LightmapUpdateEvent event, WorldClient world, BufferedImage image) {
		...
		for (int i = 0; i < 256; i++) {
			int skyPosX = (int) (sunBrightness * (image.getWidth() - 2));
			if (world.getLastLightningBolt() > 0) {
				skyPosX = image.getWidth() - 3 + world.getLastLightningBolt();
			}
			int blockPosX = (int) ((event.getTorchFlicker() + 1.0f) * image.getWidth());
			...
			color[0] = MathHelper.clamp(color[0], 0f, 1f);
			color[1] = MathHelper.clamp(color[1], 0f, 1f);
			color[2] = MathHelper.clamp(color[2], 0f, 1f);
